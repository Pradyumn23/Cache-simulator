...
char *make_block(int block_size) {
  ...
  char *blockptr = (char*) malloc(sizeof(int)*block_size);
  return blockptr;
...
Line *make_lines(int line_count, int block_size) {
 ...
 Line *theLine = (Line*)malloc(sizeof(Line)*line_count);
  int k ;
  for(k = 0; k < line_count ; k++){
  (theLine+k)->accessed = make_block(block_size);
    (theLine+k)->valid = 0;
    (theLine+k)->block_size = block_size;
    (theLine+k)->tag = 0;
  }
  return theLine;
}
...
Set *make_sets(int set_count, int line_count, int block_size) {
  ...
  //   make and initialize the line and blocks.
   Set  *theSet = (Set*)malloc(sizeof(Set)* set_count);
  int k ;
  for(k = 0; k < set_count ; k++){
  (theSet+k)->lines = make_lines(line_count, block_size);
    (theSet+k)->line_count = line_count;
    (theSet+k)->lru_queue = NULL;
  }
  return theSet;
}
